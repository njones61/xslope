...
def vp046():
    ...
    Stage 2 (steady seepage, full reservoir) is now BUILT — see build_vp046.py,
    which solves its own FE seepage (u='seep') from the conductivity ratios Baker
    publishes (equal clays, 10:1 anisotropy; the absolute Ks is FS-irrelevant) and
    gives Spencer 7.086 / Bishop 7.093 vs Slide 7.003, Baker 6.98.

    Stage 3 (rapid drawdown, Slide Spencer 2.181; Baker Fs=2.18) stays gated: its
    undrained strength is Baker's Fig. 14 (generated by the STRNGH routine from the
    Fig. 11 stress paths), a genuinely 2-D contour field whose near-surface value
    varies ~6x with the embankment surcharge. Reduced to a per-material 1-D
    cu-vs-elevation function (all XSLOPE or Slide's <.fn6> can carry), the factor of
    safety swings 1.10-3.11 with the anchor choice (2.40-2.70 for stepped zones), so
    2.181 cannot be reached without tuning to the target -- an honest stop, not a
    lock. (The <.fn6> files ship only with a Slide2 install; absent from every held
    archive -- the RS2 .fez "Slide2 Import" set skips #046.)"""
    sd = load_slope_data(ACADS_1A)
    base = dict(sd['materials'][0])
