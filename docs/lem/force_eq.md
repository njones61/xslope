...
Solving this 2x2 system gives $N'$ and $Z_{i+1}$ in closed form. Because $b$ depends on the incoming side force $Z_i$ linearly, the outgoing side force is an affine function of the incoming one, $Z_{i+1} = p_i + q_i Z_i$, with $p_i$ and $q_i$ built from the entries of $A$ and $b$. That makes the slice sequence a linear recurrence, which **xslope** evaluates for all slices at once from cumulative products and sums rather than inverting a matrix slice by slice.
...
$Z_{n+1}(F)$ has more than one root on many surfaces. The recurrence divides by the determinant of $A$, which reduces to $\cos(\alpha_i - \theta_{i+1}) + \tan\phi_i \sin(\alpha_i - \theta_{i+1}) / F$ and so vanishes at $F = -\tan\phi_i \tan(\alpha_i - \theta_{i+1})$ on every slice whose base is inclined more than $90°$ away from its side force. Each of those poles separates one root of the residual from the next, and only one of them describes a body of soil: the rest carry side forces of hundreds of times the weight of the sliding mass, and sit so close to a pole that the slice solution that produced them is meaningless. **xslope** therefore scans $F$ from 0.05 to 10, brackets every sign change, and tests each root against three measures — the determinant above stays clear of zero, the largest side force stays within the total driving load on the sliding mass, and no more than half the slices carry base tension. The driving load is the mass's own weight plus every load applied to it — the distributed and line loads, the seismic force and the tension-crack water force — so a bearing-capacity section carried by a surcharge at a nominal unit weight is judged against the load it actually carries rather than against a weight of nearly zero. The moment-equilibrium factor of safety on the same slices — Bishop's method on a circular surface, Spencer's on a non-circular one — is then solved once and used twice. A root standing more than twice below it is refused: that is what the spurious branch does even where it looks clean on the three measures above, and on a surface whose only surviving root is that branch nothing else catches it. The screen is one-sided, because these methods read *above* the moment answer by construction and on a submerged $\phi = 0$ section read far above (see [Conventions and Limitations](#conventions-and-limitations) below). Among the roots that remain, the one nearest the moment answer is reported, nearness measured as a ratio, and the roots passed over are named in the solution's warnings with the measure each failed. Where none passes, the method reports that it has no admissible answer on the surface rather than substituting another method's. Tension in the side forces themselves is reported rather than refused; see [Interslice tension](overview.md#interslice-tension).
