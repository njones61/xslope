...
$Z_{n+1}(F)$ has more than one root on many surfaces. The recurrence divides by the determinant of $A$, which reduces to $\cos(\alpha_i - \theta_{i+1}) + \tan\phi_i \sin(\alpha_i - \theta_{i+1}) / F$ and vanishes at $F = -\tan\phi_i \tan(\alpha_i - \theta_{i+1})$ on any slice whose base is inclined more than $90°$ away from its side force. Each pole separates one root from the next, and only one root describes a body of soil. **xslope** scans $F$ from 0.05 to 10, brackets every sign change, and tests each root on three measures: the determinant stays clear of zero, the largest side force stays within the total driving load on the mass (its weight plus every applied load), and no more than half the slices carry base tension. A root standing more than twice below the moment-equilibrium factor of safety on the same slices (Bishop's on a circular surface, Spencer's on a non-circular one) is refused as well, since these methods read above the moment answer by construction. Among the roots that remain, the one nearest the moment answer is reported, and the roots passed over are listed in the solution warnings with the measure each failed. Where none passes, the method reports that it has no admissible solution on the surface rather than substituting another method's. Tension in the side forces is reported rather than refused; see [Interslice tension](overview.md#interslice-tension).
