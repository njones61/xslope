...
The Fellenius normal is not iterative and has no $m_\alpha$, so the only check
**xslope** applies to it is on base tension: an answer with more than half its base
normals in tension is refused. Fewer are reported in the solution warnings, as they
are by every other method.
