...
| **Polygons** | Polygon regions (for polygon-based models) — material zones, SSR overlays, mesh refinement regions, and joint-network regions. |
...
| **Joints** | Joint lines — slip surfaces with no reinforcement in them (FEM only); whole sets of them through [Build network](#build-network). |
...
empty. A whole set of joint lines at once — bedding at a spacing, a conjugate
pair, a blocky mass — is generated by [Build network](#build-network), below.
...
existing joint lines in gray, and the line under it counts them. Nothing is written
until OK.

![Build network dialog](images/editing_joint_network_dialog.png)

**The kinds.** A *parallel set* is one orientation at one spacing: a dip
(counter-clockwise from horizontal, so 0 is flat, 90 is vertical, and a set
descending to the right is negative), the perpendicular spacing between traces, and
an offset that slides the whole set across its own normal. A *trace length* with a
*gap* makes the set discontinuous — each trace exists for that length, stops for a
rock bridge, and resumes. *Cross-jointed* is two of those crossing, each with its
own dip, spacing and offset; the two dips must differ, because two sets at one dip
lie on top of one another and the mesh split needs material between them.
*Voronoi* is a blocky mass, described by its block size and a seed rather than by
an orientation: the same two numbers reproduce the same network exactly.

**Where the set exists.** The whole section, one material (every zone carrying it),
or a **joint region** — a polygon whose Type is `joints`, drawn in the Polygons
editor (see [joint regions](../usage/input_template.md#joint-regions)), which is how a set is confined to ground no material
boundary draws. Any of those can be cut to an **elevation band**, which is what
"the sandstone above elevation 40" asks for; either end may be left open. A trace
that would run along the outside of the section is dropped, because the mesh splits
a joint by putting material on both sides of it and there is none out there; a
trace along a band's own edge is kept, since a band is a line drawn through
material.

**Editing a set.** Every row the dialog writes is labeled with the set's own
parameters — `bed-01|par|dip=25|s=2.5|reg=poly:North block` — so the set stays one
thing after it is written:

![Joints editor after a network is built](images/editing_joints_network_rows.png)

Select any row of a set, press **Build network…** again, and the dialog opens on
that set filled in as it was built. Change the spacing, press OK, and the set is
regenerated where it was: its own rows are replaced, and the other sets and any
hand-entered joint lines are left exactly where they are. A hand-typed label is
never part of a set, so a line you entered yourself is never regenerated away.
