...
equilibrium engines do not read the sheet — so this editor carries no engine
toggle: there is nothing to toggle between. A whole set of joint lines at once —
bedding at a spacing, a conjugate pair, a blocky mass — is generated by
[Build network](#build-network), below.
...
The description itself is not stored, so nothing reopens a set: to change a
network, select one of its rows, press **Remove set**, and build another. (Within
a session the dialog opens on the parameters you last built with, under a fresh
name, so that is one number to change rather than nine to re-enter.) A hand-typed
label belongs to no set, so a line you entered yourself is never caught up in
removing one.
