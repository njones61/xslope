...
| # | Tutorial | What it covers | Analysis | Features | Time |
| :---: | --- | --- | --- | --- | :---: |
| 1 | [The AI Assistant](w01_ai_assistant.md) | Studio's built-in assistant put through eight requests on the reinforced slope of LEM-8 — setting it up with a provider, a model and a key, and what a turn costs; then building the model from nothing but its drawing, editing its face, its loads and its reinforcement, a sweep the Parametric dialog offers and a sweep it does not, material stiffnesses suggested and then run through strength reduction, a question answered out of this documentation, three transcription errors found in a broken file, and the analysis report — with what to check after each one | Any | AI assistant, provider setup, images, model edits, parameter studies, strength reduction, documentation, analysis report | ~30 min |
