...
| 13 | [A Rock Slope (Hoek-Brown)](lem13_rock_slope.md) | A 10 m rock cut whose strength is not a cohesion and a friction angle but four field observations — the intact strength, the Geological Strength Index, the intact constant and the blast disturbance — solved by Spencer's method and then by strength reduction on the same file, then read once more with its intact strength typed in the megapascals the literature quotes, and finally a second, much stronger rock mass stated in the normalized form rock-slope charts use | Limit equilibrium, finite element (strength reduction) | one material, Hoek-Brown, GSI, disturbance factor, instantaneous tangent, circular search, strength reduction, quadratic triangles, model checks | ~20 min |
...
| 3 | [Factor of Safety vs Time](combo03_fs_vs_time.md) | SEEP-3's cored earth dam given strengths and run for stability against every saved frame of its 45-day drawdown — how `u = seep` and Run LEM's **Seepage time** group send one instant of a transient march to a limit equilibrium search, why the search window on the circles sheet is what keeps the curve on one mechanism, and what the curve says that a single analysis cannot: 1.831 at full pool, a minimum of 1.496 on day 30 — 17 days before the pool stops falling — 1.576 at the end of the drawdown, and a slow recovery to 1.768 that never regains full pool | Seepage + limit equilibrium | two materials, transient seepage, u = seep, saved frames, seepage time, FS vs time, Spencer, circular search, search window, minimum slip depth, automatic water loads | ~30 min |
