...
| 13 | [A Rock Slope (Hoek-Brown)](lem13_rock_slope.md) | A 10 m rock cut whose strength is not a cohesion and a friction angle but four field observations — the intact strength, the Geological Strength Index, the intact constant and the blast disturbance — solved by Spencer's method and then by strength reduction on the same file, read once more with its intact strength typed in the megapascals the literature quotes, and then swept across the Geological Strength Index and the blast-disturbance factor to find which of the four field inputs the answer turns on | Limit equilibrium, finite element (strength reduction) | one material, Hoek-Brown, GSI, disturbance factor, instantaneous tangent, circular search, strength reduction, quadratic triangles, parametric study, design sweep, model checks | ~25 min |
...
| 3 | [Transient Seepage: Reservoir Drawdown](seep03_reservoir_drawdown.md) | A 22 m dam with a granular shell and a clay core, whose reservoir is lowered to the tailwater elevation over 45 days — the storage properties that decide how much water a soil gives up as its head falls, the full-pool steady solution the transient run starts from, a pool schedule entered as a named time series and bound to the upstream boundary, and the three ways a transient answer is read: frames on a play bar, a head history where the core's head passes above the shell's, and an outflow ten times the steady discharge drawn entirely from storage | Seepage | two materials, transient seepage, specific storage, specific yield, time series, reservoir boundary, initial condition, saved frames, head history, mass balance | ~30 min |
...
| 2 | [Reinforcement: LEM vs FEM](fem02_reinforcement.md) | Six layers of geogrid in a 24 ft sand fill solved by Spencer's method and then by strength reduction — the two limits every reinforcement model carries, bond and rupture, and what each engine does with them; the axial stiffness the finite element run refuses to start without; the per-trial iteration budget without which both runs return the same number; the elastic-perfectly-plastic run against the peak-residual one, read out of shear strain and each layer's own force profile; and two measurements on how much the modeling choices are worth — the soil-to-bar stiffness ratio, and a residual capacity that lands on three values rather than a curve | Finite element | two materials, reinforcement lines, capacity envelope, pullout length, bar elements, axial stiffness, elastic-perfectly-plastic, residual capacity, strength reduction, quadratic triangles, thin zones, iteration budget, 1D details, shear strain | ~30 min |
...
| # | Tutorial | What it covers | Analysis | Features | Time |
| :---: | --- | --- | --- | --- | :---: |
| 1 | [Seepage &rarr; LEM &rarr; FEM](combo01_seepage_stability.md) | The Johnson Reservoir dam run three ways from one file — a seepage solution on a quadratic mesh, then a Spencer search and a strength reduction on that same mesh and the same pore pressures — the element type a shared mesh has to be chosen for, the materials table's `u` column that decides whether the stability engines read the seepage answer at all and what leaving it on `none` costs, and the staleness rule that stops the three results belonging to three different models | Seepage + limit equilibrium + finite element | three materials, unconfined seepage, shared mesh, quadratic triangles, pore-pressure option, u = seep, Spencer, circular search, strength reduction, shear strain, stale results | ~20 min |
| 2 | [Rapid Drawdown](combo02_rapid_drawdown.md) | The same dam with its reservoir lowered 50 ft, from elevation 160 to a residual pool at 110 — the Duncan, Wright and Wong three-stage procedure and its `d` / `ψ` undrained envelope, then its two water states supplied three ways, each searched from the same starting circle: a sketched piezometric pair, two steady seepage solutions from a second boundary set, and two frames of a transient run, reading 1.181, 1.195 and 1.016 — with the drained runs that bracket them, and a sweep of the core's undrained intercept that hands the governing stage from stage 2 to stage 3 | Limit equilibrium + seepage | three materials, rapid drawdown, three-stage procedure, Kc = 1 envelope, d and ψ, piezometric lines, second boundary set, transient seepage, stage times, automatic water loads, Spencer, circular search, parametric sweep | ~35 min |
| 3 | [Factor of Safety vs Time](combo03_fs_vs_time.md) | Two dams run for stability against every saved frame of a transient drawdown. Part 1 gives SEEP-3's cored earth dam drained strengths — how `u = seep` and Run LEM's **Seepage time** group send one instant of a transient run to a limit equilibrium search, how to place one starting circle per face so a curve spanning a full reservoir and an empty one can report either, and a minimum of 1.331 on day 35 on the upstream face against 1.531 on the downstream face at full pool. Part 2 turns the same sweep into a three-stage rapid drawdown at every instant of COMBO-2's Johnson Reservoir transient run: 1.016 on day 50, which is COMBO-2's own answer on the same circle, against 1.035 for the drained analysis of that instant — with both curves on one figure and the reason the gap is narrowest where the slope is weakest | Seepage + limit equilibrium (drained, and rapid drawdown) | two materials, three materials, transient seepage, u = seep, saved frames, seepage time, FS vs time, rapid drawdown, three-stage procedure, Kc = 1 envelope, d and ψ, stage times, Spencer, circular search, starting circles, minimum slip depth, automatic water loads | ~35 min |
...
| # | Tutorial | What it covers | Analysis | Features | Time |
| :---: | --- | --- | --- | --- | :---: |
| 1 | [The AI Assistant](w01_ai_assistant.md) | Studio's built-in assistant put through eight requests on the reinforced slope of LEM-8 — setting it up with a provider, a model and a key, and what a turn costs; then building the model from nothing but its drawing, editing its face, its loads and its reinforcement, a sweep the Parametric dialog offers and a sweep it does not, material stiffnesses suggested and then run through strength reduction, a question answered out of this documentation and the verification corpus, a broken file diagnosed one input at a time, and the analysis report — with what to check after each one | Any | AI assistant, provider setup, images, model edits, parameter studies, strength reduction, documentation, analysis report | ~30 min |
| 2 | [Bringing in Models from CAD and Other Programs](w02_import.md) | The three importers walked end to end — a layered DXF section of a highway embankment mapped layer by layer through the import wizard, with the seven targets a layer can take and the properties no CAD file can hold typed in afterward (1.185 on a surface tangent to the dense sand); a GeoStudio SLOPE/W verification dam that arrives whole, including one of SLOPE/W's own solved trial circles, so the two programs can be compared on one surface (1.939 against 1.934); and a Slide2 scenario that arrives with no failure surface at all, because Slide2 stores a search rather than a surface (1.372 against 1.360 once a starting circle is added) — with the caveat list every import reports, read against what each source model defines | Limit equilibrium | DXF import, layer mapping, material zones, piezometric line, distributed load, failure circles, GeoStudio import, analysis picker, imported trial circle, Slide2 import, scenario picker, import caveats, Spencer, circular search | ~25 min |
