...
| 3 | [Piles: LEM vs FEM](fem03_piles.md) | Two rows of drilled shafts and a sheet pile wall, each put through both engines — why a plane-strain finite element model has no gap between piles and turns a discrete row into a continuous wall, seen in the mechanism it forces; what the row is worth to each engine on the same file; a spacing sweep in which the limit equilibrium answer swings 36% and changes which surface governs while the strength reduction answer does not move, even as each shaft reaches its moment capacity; the one benchmark with a published three-dimensional answer; and then the case the finite element engine is right for — a wall entered as a continuous member at spacing 1, meshed and re-solved with its seepage field, and read for the moment, shear, deflection and soil reaction no limit equilibrium analysis can report | Finite element | piles, Ito &amp; Matsui, pile spacing, beam elements, smeared stiffness, plane strain, sheet pile wall, continuous member, strength reduction, quadratic triangles, steady seepage, shear strain, 1D details, moment and shear profiles | ~75 min |
