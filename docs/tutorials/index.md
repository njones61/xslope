...
| 2 | [Rapid Drawdown](combo02_rapid_drawdown.md) | The same dam with its reservoir taken from elevation 160 to elevation 100 — the Duncan, Wright and Wong three-stage procedure, its `d` / `ψ` undrained envelope and the textbook hand calculation behind it, then its two water states supplied three ways on one circle: a sketched piezometric pair, two steady seepage solutions from a second boundary set, and two frames of a transient march, reading 1.301, 1.564 and 1.314 — with the drained runs that bracket them, and a sweep of the core's undrained intercept that hands the governing stage from stage 2 to stage 3 | Limit equilibrium + seepage | three materials, rapid drawdown, three-stage procedure, Kc = 1 envelope, d and ψ, piezometric lines, second boundary set, transient seepage, stage times, automatic water loads, Spencer, parametric sweep | ~35 min |
