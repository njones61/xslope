...
| 3 | [Transient Seepage: Reservoir Drawdown](seep03_reservoir_drawdown.md) | A 22 ft dam with a granular shell and a clay core, whose reservoir is emptied to the tailwater over 45 days — the storage properties that decide how much water a soil gives up as its head falls, the full-pool steady solution the march starts from, a pool schedule entered as a named time series and bound to the upstream boundary, and the three ways a transient answer is read: frames on a play bar, a head history where the core's head passes above the shell's, and an outflow nearly eight times the steady discharge drawn entirely from storage | Seepage | two materials, transient seepage, specific storage, specific yield, time series, reservoir boundary, initial condition, saved frames, head history, mass balance | ~30 min |
