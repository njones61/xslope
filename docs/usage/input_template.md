...
>>Tip: Rotational boundary condition at the pile tip (bottom node) for FEM analysis. **free** (default) = the tip rotates freely, which is correct for a shaft the surrounding soil restrains; **fixed** = zero rotation at the tip, modeling a shaft socketed into rock or bearing on a fixed base. Blank or omitted = free.<br>
>>Head and Tip each constrain the rotation degree of freedom at that node and nothing else — the translations remain with the boundary conditions and the soil around the pile. Neither has any effect on LEM analysis.<br>
