...
>>Tip: Boundary condition at the pile tip (bottom node) for FEM analysis. **free** (default) = the tip is restrained only by what surrounds it, the soil or the model boundary; **pinned** = the tip's translations are held and its rotation is free, a shaft bearing on a hard stratum inside the mesh; **fixed** = translations and rotation held, a shaft socketed into rock. Blank or omitted = free.<br>
>>Head constrains only the rotation at the top node; the head's translations remain with the soil around the pile. Neither column has any effect on LEM analysis.<br>
...
the run **duration** draws a load-time warning, since it would never be reached — and so does a
**stability_time** beyond it.
