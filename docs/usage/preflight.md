...
| **Surfaces** | A model with no failure surface at all; a method that cannot use the selected surface family; a model carrying both families where the run did not say which; a circle whose **Depth** sits below the base of the model and that cuts no failure surface inside it — reported against a search, whose refinement moves off its seeds and reaches the critical surface anyway, and refused where that circle *is* the run's surface |
