...
### VP42: Baker & Leshchinsky safety-map dam — reservoir-load statics {#vp42}
...
**The gap is a reservoir-load statics difference, established on an identical surface.** Three independent references cluster tightly: Slide **1.925**, Baker **1.91**, and SLOPE/W's own solved critical Spencer **1.934** (read from the vendor `.gsz`; [§2.25](geostudio.md#gs-2-25)). The `.gsz` is decisive because it can be inspected: it carries the identical geometry, the same three Mohr-Coulomb materials at their **total** unit weights, and a piezometric line within ~1–2 m of XSLOPE's everywhere — no buoyant weights anywhere. Evaluated on SLOPE/W's *own* critical circle (center 234.9, 207.1, R = 204.4), XSLOPE reads Spencer **1.691** — ~13% below SLOPE/W's 1.934 on the *same surface, same geometry, same water*; every XSLOPE method sits below (OMS 1.573, Bishop 1.651), and an unconstrained XSLOPE search finds a lower true minimum (≈ 1.41). The divergence tracks the dominant reservoir (ponded-water) load: 13 of the 34 slices on that surface lie under the submerged toe, removing the reservoir load swings XSLOPE by ~1.0 in FS, and the slide-mass weight books differ (SLOPE/W folds the ponded water into column weight; XSLOPE applies it as a separate face traction). It is a slice-level statics difference in how a large external water load is carried — a regime in which XSLOPE's own method notes already warn that the simplified methods (OMS, Janbu) cannot balance the load.
...
*The XSLOPE values are regression-locked as our own consistency guard; they are deliberately **not** expected to match the published cluster. XSLOPE reads systematically low on this heavily reservoir-loaded, inclined-phreatic surface, and the identical-circle comparison above localizes the reason to the reservoir-load statics — not a weight convention (both sides use total weight + pore pressure) and not a phreatic-geometry guess (XSLOPE's piezometric line matches the `.gsz` to within ~1–2 m). This is the corpus's clearest case where XSLOPE and three independent LE references, sharing the same effective-stress convention, still differ ~13–19% on a surface governed by a large external water load.*
