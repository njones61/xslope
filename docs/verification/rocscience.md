...
fluid pressure support type) or a source artifact not yet in hand (VP46's discrete drawdown
...
| 37 | Slope, homogenous, distributed load, back analysis of required support force and length | *blocked* | XSTABL (1999) back-analysis of the support force that raises a cohesionless slope to FS = 1.5 (Slide 351 kN / XSTABL 345 kN). The method maps cleanly onto [`design()`](../lem/design.md) — sweep the support force to a target factor of safety — and Figure 37.1 is fully coordinate-labeled with the 40 kN/m² crest load, but the Slide2 manual prints no material properties (φ, γ) for the soil and the XSTABL Reference Manual source is not in hand, so the inputs are incomplete and no benchmark can be built without guessing them. Part (b) — the minimum reinforced-zone length via an elevated-friction zone — additionally needs a variable-length material zone that a single-scalar design sweep does not express. *Archive recheck (2026-07-19): no VP37 model file exists in any vendor archive we hold — the RS2 `.fez` "Slide2 Import" set skips #037, and the gsz/GeoStudio corpora carry no such problem — so the missing φ/γ can only come from the XSTABL Reference Manual (Sharma 1996) or the Slide2-install verification model.* |
...
| [42](#vp42) | Dam, (3) materials, water table, ponded water, tension crack | **built** | [vp042.xlsx](../files/rocscience/vp042.xlsx). Baker & Leshchinsky (2001) safety-map clay-core dam — the corpus's reservoir-load statics finding (XSLOPE reads ~13–19% below Slide/Baker/SLOPE/W, which share XSLOPE's total-weight + u convention; the difference is the large reservoir-load statics, not a weight convention; see section). Also [SLOPE/W §2.25](geostudio.md) — same problem in the GeoStudio corpus. |
...
### VP42: Baker & Leshchinsky safety-map dam — an open reservoir-loaded statics gap {#vp42}

Slide #42 / Baker & Leshchinsky (2001): the safety-map clay-core dam — granular fill (c' = 0, φ' = 40°, γ = 21.5) around a diamond core (c' = 20, φ' = 20°, γ = 20) on a hard base (c' = 200, φ' = 45°), a half-full upstream (right-side) reservoir, phreatic dropping through the core to a tailwater exit at the downstream toe, and a 5-m cracked layer at the crest modeled as a dry tension crack. Geometry is fully labeled in Slide's figure (all six core vertices); B&L's own Fig. 5(a) — the paper is in the reference library — supplies the phreatic (flat through the shell, descending through the core, half-height reservoir).

The paper states its method explicitly: pore pressures are *"evaluated using the vertical distance between the phreatic surface and the slice center,"* and the material weights are total unit weights — the clay core is given as *"{c′ = 20, φ′ = 20°, γ<sub>T</sub> = 20} where {c′, φ′} are effective strength parameters and γ<sub>T</sub> represents the total unit weight."* It reports a global minimum F<sub>min</sub> = 1.91 by Spencer's method. So the published analysis uses **total unit weights and explicit pore pressures from the phreatic surface — the same effective-stress convention XSLOPE uses.** The XSLOPE-vs-published gap on this problem is therefore *not* a buoyant-weight convention on the published side. Its cause has not been isolated: the two candidate mechanisms — a reservoir-load slice-statics difference and a phreatic/pore-pressure input difference — are each ruled out by the controlled checks below, so the gap stands as an unexplained program-to-program statics difference.

**What XSLOPE computes.** Total unit weights, pore pressures from the piezometric line, and the reservoir as an explicit hydrostatic load on the submerged face. On Slide's printed critical circle this gives Spencer **1.572**; on Baker's surface, **1.792**. Every input was audited — slice-level pond and pore pressures match hand calculations exactly.

**An identical-surface gap that no identified input explains.** Three independent references cluster tightly: Slide **1.925**, Baker **1.91**, and SLOPE/W's own solved critical Spencer **1.934** (read from the vendor `.gsz`; [§2.25](geostudio.md#gs-2-25)). The `.gsz` is decisive because it can be inspected: it carries the identical geometry, the same three Mohr-Coulomb materials at their **total** unit weights, and — checked node-by-node along the failure surface — a piezometric line that matches XSLOPE's to within ~0.2 m over the submerged toe and reservoir (the zone carrying ~92% of the pore force) and ~0.6 m at worst near the core transition, with XSLOPE's line marginally the *lower* of the two. Adopting the vendor line verbatim would move Spencer by an implied ≈ 0.01 in FS — two orders of magnitude short of the gap, and the wrong sign to close it. Evaluated on SLOPE/W's *own* critical circle (center 234.9, 207.1, R = 204.4), XSLOPE reads Spencer **1.691** — ~13% below SLOPE/W's 1.934 on the *same surface, same geometry, same water*; every XSLOPE method sits below (OMS 1.573, Bishop 1.651), and an unconstrained XSLOPE search finds a lower true minimum (≈ 1.41).

**Two candidate mechanisms, both ruled out.** (1) *Reservoir-load statics.* XSLOPE carries the reservoir as a hydrostatic traction normal to the submerged face. For a fully submerged still-water slope this treatment is exact: on an identical circle it reproduces the closed-form dry-buoyant-weight solution (γ′ = γ<sub>sat</sub> − γ<sub>w</sub>, no water, no load) to within 0.006 in Bishop and Spencer FS. The alternative of folding the ponded-water column into vertical slice weight is genuinely inequivalent — it differs by tens of percent and can drive the base into non-physical tension — and it *lowers* FS, so it cannot account for SLOPE/W reading higher. (2) *Phreatic input.* FS here is sensitive to the phreatic — a uniform 1-m lowering of the line raises Spencer ≈ 0.075 — which is precisely why the line was audited against the vendor `.gsz`; it matches to within ~0.2 m along the surface (above), so this lever does not bite, and most of the apparent sensitivity lives in the reservoir zone whose level (el 30) is pinned by the water surface and shared by both programs. Removing the reservoir entirely swings XSLOPE by ~1.0 in FS, confirming the load dominates the balance — but its statics are validated and its phreatic matches, so the residual ~13% is an unexplained slice-statics difference between the two programs on this deep, steep, mostly-submerged circle, in a regime where XSLOPE's own method notes already warn that the simplified methods (OMS, Janbu) cannot balance the load.

**Input files:** [vp042.xlsx](../files/rocscience/vp042.xlsx)

| Surface (Spencer) | XSLOPE | Published / vendor |
|---|---|---|
| Slide's critical circle | 1.572 | Slide **1.925** |
| Baker's noncircular surface | 1.792 | Baker & Leshchinsky **1.91** |
| SLOPE/W's own critical circle | 1.691 | SLOPE/W own solve **1.934** |

*The XSLOPE values are regression-locked as our own consistency guard; they are deliberately **not** expected to match the published cluster. XSLOPE reads systematically low on this heavily reservoir-loaded, inclined-phreatic surface, and the identical-circle comparison above leaves the cause open: it is neither a weight convention (both sides use total weight + pore pressure), nor a phreatic-geometry difference (XSLOPE's piezometric line matches the `.gsz` to within ~0.2 m where it counts), nor an error in the reservoir-load statics (exact against a buoyant-weight oracle). This is the corpus's clearest case where XSLOPE and three independent LE references, sharing the same effective-stress convention and the same water, still differ ~13–19% on a surface governed by a large external water load.*
