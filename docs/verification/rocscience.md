...
| 38 | Excavated slope, homogenous, finite element groundwater seepage analysis, matric suction | *blocked* (u-source) | Ng & Shi (1998) Hong Kong cut: FE steady-state seepage produces matric suction (negative pore pressure) above the water table, and the stability uses the extended Mohr-Coulomb criterion with a *distinct* suction-strength angle φᵇ = 15° (τ = c′ + (σ − uₐ)tan φ′ + (uₐ − u_w)tan φᵇ); soil c′ = 10 kPa, φ′ = 38°, γ = 16 kN/m³. Published Bishop FS for right-side head H = 61/62/63 m: Slide 1.621 / 1.538 / 1.407, Ng & Shi 1.636 / 1.527 / 1.436. The **strength-side gap is now closed**: `generate_slices(suction_phi_b={material: φᵇ}, suction_cap=…)` prices the *unclamped* base suction s = max(0, −u) as an apparent cohesion c_suction = s·tan φᵇ added to the resisting c·Δℓ term while the effective-normal term keeps u clamped at 0 — algebraically Slide2's extended Mohr-Coulomb with uₐ = 0. Opt-in, off by default (bit-identical to the clamped baseline). What remains is the **u-source**: the vendor `.fez` carries only the *unsolved* FE seepage model (mesh + boundary heads + SWCC live in the `.slw`; its initial-pore-pressure block is all zero and the results view `.p2mrv` is empty), so the negative-u suction field that produces the published FS is not importable — reproducing it means regenerating Ng & Shi's unsaturated steady-state seepage, which is out of scope here and overlaps the RS2 seepage-field import track (cf. RS2-28). A φᵇ = 15° vs clamped-baseline run on the reconstructed geometry with a placeholder water table confirms suction raises FS and that an *uncapped* hydrostatic suction is unphysical (it grows without bound below the phreatic surface, doubling apparent cohesion — a `suction_cap` in the SWCC air-entry range, ≈ 30–43 kPa here, is required); the absolute FS does not reproduce the published values without the FE seepage field. |
