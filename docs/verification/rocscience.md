...
(VP46's stage-2/3 seepage and undrained-strength fields, which Baker (1993) prints only as
contour maps) — not an open-ended backlog. Everything else is built and regression-locked; the
...
| [46](#vp46) | Dam, (2) materials, rapid drawdown, finite element groundwater seepage analysis, ponded water | **partial** (stage 1 built) | [vp046.xlsx](../files/rocscience/vp046.xlsx). Baker (1993) three-stage validation dam. **Stage 1 (dry)** now built: the c′ = 0 downstream natural-clay face is a 4H:1V infinite-slope skin, so XSLOPE's circular search lands on the closed form FS = tan 32°/tan 14.04° = **2.50** (Spencer/Bishop) — exactly the manual's theoretical 2.5, vs Slide's min-depth-5m noncircular Spencer 2.534 (−1.4%) and Baker 2.41. Stages 2 (steady seep, Spencer 7.003) & 3 (rapid drawdown, Spencer 2.181) stay vendor-gated even with Baker (1993) now read: the paper prints the fields they need only as FLAC-derived contour maps — steady-state pore pressures (Fig. 12b) for stage 2, undrained strength S(x,y) (Fig. 14) for stage 3 — not as tabulated functions, and gives no numeric permeability (clays "approximately equal", 10:1 anisotropy, p. 32). So Slide's stage-2 field rests on Rocscience's *estimated* seepage parameters (manual Table 46.1) and stage 3's per-material `.fn6` functions are a digitization of Fig. 14 that ships only with a Slide2 install (absent from every held archive — the RS2 `.fez` "Slide2 Import" set skips #046, and the RS2-native "#046 (cz=…)" is a different cohesion-with-depth problem). The paper does corroborate the targets: Baker Fs = 2.41 / 6.98 / 2.18 for stages 1 / 2 / 3. |
...
**Stages 2 and 3 remain vendor-gated — reading Baker (1993) in full does not unblock them.** Both stages hinge on a spatial field that Baker computed with a finite-difference continuum program (FLAC) and prints only as a contour map, never as a tabulated function or equation: stage 2 (steady seepage, Slide Spencer 7.003) needs the steady-state pore-pressure field (Baker Fig. 12b, 10 kPa contours), and stage 3 (rapid drawdown, Slide Spencer 2.181) needs the undrained-strength distribution S(x,y) (Baker Fig. 14, 20–120 kPa contours), generated point-by-point by his STRNGH routine from the Fig. 11 stress paths. Stage 2 is additionally under-determined because Baker publishes no numeric permeability, so Slide's seepage rests on Rocscience's estimated parameters (manual Table 46.1: Ks = 7×10⁻⁵ for both clays, K2/K1 = 0.1, natural-clay unsaturated a = 0.06, n = 2). Stage 3's per-material `.fn6` discrete functions are Slide's own digitization of Fig. 14 — which the manual says must be "manually extracted from the author's data" — and ship only with a Slide2 install, confirmed absent from every held archive (the RS2 `.fez` "Slide2 Import" set skips #046; the RS2-native "#046 (cz=…)" is a different cohesion-with-depth problem under RS2's own numbering). The paper does corroborate the published factors of safety — Baker Fs = 2.41 (empty reservoir, §6.4.1), 6.98 (steady state, §6.4.2 + Table I), and 2.18 (rapid drawdown, direct approach, §6.4.3.1 + Table I) — which bracket Slide's 2.534 / 7.003 / 2.181.
