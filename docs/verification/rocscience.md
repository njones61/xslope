...
| 29 | Submerged slope, homogenous, probabilistic analysis, water table | partial | [vp029.xlsx](../files/rocscience/vp029.xlsx). Duncan (2000) LASH terminal — the canonical TSPM problem. Inputs built (geometry labeled; Duncan's surface both as the printed axis/radius circle and as a bright-red trace validated against the printed endpoints; `cp` strength; σ_γ=3.3, σ_cp=1.2). Two reconstructions of the surface agree (Spencer 1.247/1.237) and the explicit-water vs buoyant-weight check agrees to 0.7% (static water — the formulations must and do coincide), but both sit 7% above Slide 1.157 / Duncan 1.17 — the su-datum semantics (extrapolation above el. −20 vs clamping) are the open question. Duncan (2000) and D&W (2014) §13.5.6 are now in the reference library and CONFIRM the inputs: the su profile (Fig. 13.1 average line = 100 psf at el. −20 + 9.8 psf/ft), γ′ = 38 ± 3.3, and the TSPM table (strength ΔF 0.31, γ ΔF 0.20, σ_F 0.18, PF 18%). The residual ~7% on the deterministic FS is a surface-digitization question (Slide's printed axis is a moment axis, not a circle center — the drawn surface sits up to 17 ft above the equivalent circle at mid-span, yet both read ~1.24 in XSLOPE). Notably the published probabilistic answers span PF = 14% (Slide LHS) / 18% (Duncan 2000 TSPM) / 30–33% (D&W 2014, wider σ_su) — the σ-input choice dwarfs the estimator difference, which is the real lesson for TSPM-vs-Monte-Carlo comparisons. `reliability(search=False)` was added for this problem — fixed-surface reliability on a prescribed slip surface. |
