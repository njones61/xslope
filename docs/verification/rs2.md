...
but unlocked — see [RS2-67](#rs2-67)), and the transient solver independently reproduces RS2's own
90 h drawdown field as a fidelity cross-check. Where a
...
| [28](#rs2-28) | Excavated slope, FE groundwater and matric suction (Ng & Shi 1998) | **built** (blocked) | [rs2_28a](files/rocscience/rs2_28a.xlsx) / [b](files/rocscience/rs2_28b.xlsx) / [c](files/rocscience/rs2_28c.xlsx). Slide2 [VP38](rocscience.md#vp38). RS2 SSRM 1.64 / 1.55 / 1.41 (manual Part 1 §28 — *not* the "1.56/1.46/1.32" quoted elsewhere). Blocked: the vendor `.fea` ships suction OFF (`UseUnsaturated: 0`, `Phi_b: 0`), crediting it instead through effective stress at φ′; and the ≈200 kPa far-field-head pore pressure prevents the viscoplastic SSRM from converging at any F. See the section. |
...
| [38](#rs2-38) | Cohesionless embankment on saturated clay foundation | **built** | [vp074.xlsx](files/rocscience/vp074.xlsx). Sand on saturated clay (D&W Fig 7.12). SSRM 1.168 vs RS2 SSRM 1.17 (Part 4) / 1.21 (Part 2), Slide2 non-circular 1.18. |
| [41, 43](#rs2-39) | Earth embankment, infinite-slope mechanism | **built** (caveat) | [vp079.xlsx](files/rocscience/vp079.xlsx) / [vp081.xlsx](files/rocscience/vp081.xlsx). Infinite-slope skins: SSRM 1.430 (VP79, on D&W 1.44) / 1.097 (VP81, c=0 skin ~5% low). Problem 39 (VP76, Fig 7.19) is the FE-seepage sibling, deferred. |
...
| [48–55](#rs2-48) | Multi-tiered geotextile walls (Leshchinsky & Han 2004) | **built** (baseline) / partial | Slide2 [VP87](rocscience.md#vp87)–VP94. The SSRM now enforces the geotextile tensile-capacity cap; the baseline wall (vp087) verifies at SSRM 0.969 vs L&H ≈1.0 / Slide 1.04. Of the seven parametric variants, four converge (0.76–1.10, bracketing ≈1.0) and three (vp089 / vp090 / vp093) localize a shear band in the c = 0 reinforced fill — bounded via the vendor `.fez` to a fill-localization gap (not element order, not a can't-fail facing, not the vendor's T = c cutoff, which moves the factor further off), recorded with reason. |
...
| [67](#rs2-67) | Earth dam under steady & transient unsaturated seepage | **built** (6 of 6 locked: 3 vendor-match <1%, 3 own-flow regression locks) | [rs2_67a](files/rocscience/rs2_67a.xlsx) (dry) / [b](files/rocscience/rs2_67b.xlsx) (steady, own-flow) / [c](files/rocscience/rs2_67c.xlsx) (90 h downstream) / [d](files/rocscience/rs2_67d.xlsx) (90 h upstream) / [e](files/rocscience/rs2_67e.xlsx) (1500 h downstream, own-flow) / [f](files/rocscience/rs2_67f.xlsx) (1500 h upstream, own-flow). Huang & Jia (2009) homogeneous dam. The 90 h drawdown pore-pressure fields are **imported** from RS2's own computed `.fea` nodal blocks through the `u='seep'` path (RS2 mesh + nodal u → seep sidecars) — verifying the SSRM-under-transient-*u* mechanics; XSLOPE's own transient-seepage solver independently reproduces the 90 h upstream phreatic to <0.3 m (fidelity cross-check). SSRM 2.455 / 1.820 / 2.023 vs RS2 SSRM 2.48 / 1.83 / 2.04 (all within 1%); the 90 h upstream run confines the SSRM to RS2's upstream Search Area. Each snapshot field is hydrostatic below a 14-point phreatic surface (nodal residual < 10⁻³ kPa). Cases 2 (steady) and 4 (1500 h) carry no recoverable solved field, but their `.fea`/`.slw` retain the groundwater BC block; RS2 renders each — the full pool for Case 2, the fully-drained drawn-down pool (el 7.3) for Case 4 — as a steady state, so the flow is reconstructed by an own steady-seepage solve. SSRM 1.602 / 2.207 / 2.660 vs RS2 SSR 1.70 / 2.34 / 2.76 — each within the Slide2 LEM method spread but a few percent below RS2 (a structural FE/SSR formulation difference — a sandbox reproduction of RS2's own built-in permeability curve leaves the field and FS essentially unchanged), regression-locked at XSLOPE's own values with the deltas documented. |
| [68](#rs2-68) | Seismically loaded slopes | **built** | [rs2_68a.xlsx](files/rocscience/rs2_68a.xlsx) / [b](files/rocscience/rs2_68b.xlsx) / [c](files/rocscience/rs2_68c.xlsx). Loukidis, Bandini & Salgado (2003). Target is a **critical seismic coefficient** k꜀ (the k giving FS = 1), not an FS — locked via a new `critical_kc` bisection harness. Homogeneous Cases 1/2 (r<sub>u</sub> = 0.5 / dry): k꜀ 0.127–0.432 (Bishop/Spencer) on the Slide2/reference LEM to ~0.001; Case 3 (3-layer, band-riding) 0.167–0.169 vs Slide2 0.151–0.155 — high by ~10% (circular can't ride the φ = 15° band as tightly as non-circular) but inside the UB/LB bracket [0.148, 0.172] and on RS2 SSRM/FEM 0.161. |
...
| # | Problem | Status | XSLOPE file / results |
|---:|---|---|---|
| 1 | Slope, homogeneous (ACADS 1a) | **built** | → [RS2-1](#rs2-1). RS2 SSRM 0.98 vs ref 1.00 [Giam]. |
| 2 | Slope, homogeneous, tension crack (ACADS 1b) | **built** | → [P4-VP2](#p4-vp2) (own SSRM build). RS2 SSRM 1.63 vs ref 1.65 [Giam]. |
...
| 14 | Slope, homogeneous (Arai & Tagyo 1) | **built** | → [RS2-10](#rs2-10). RS2 SSRM 1.37–1.39. |
| 15 | Slope, 3 materials, weak layer (Arai & Tagyo 2) | **built** | → [RS2-11](#rs2-11). RS2 SSRM 0.41 vs Kim/Greco 0.39–0.44. |
| 16 | Slope, homogeneous, water table (Arai & Tagyo 3) | **built** | → [RS2-12](#rs2-12). RS2 SSRM 1.09. |
| 17 | Slope, homogeneous (Yamagami & Ueta) | **built** | → [RS2-13](#rs2-13). RS2 SSRM 1.32. |
| 19 | Slope, 4 materials (Greco ex. 4) | **built** | → [RS2-15](#rs2-15). RS2 SSRM 1.38 vs Greco/Spencer 1.40–1.42. |
| 21 | Homogeneous, r<sub>u</sub> (Fredlund & Krahn) | **built** | → [RS2-17](#rs2-17). RS2 SSRM 1.98 / 1.68 / 1.77. |
| 22 | Weak layer, r<sub>u</sub> (Fredlund & Krahn) | **built** | → [RS2-18](#rs2-18). RS2 SSRM 1.26 / 0.99 / 1.15. |
| 24 | Slope, 3 materials (Low 1989) | **built** | → [RS2-19](#rs2-19). RS2 SSRM 1.42 vs Low 1.44. |
| 25 | Bearing-capacity slope (Prandtl / Chen & Shao) | **built** | → [RS2-20](#rs2-20). RS2 SSRM 1.01 vs Chen & Shao 1.05. |
| 26 | Bearing-capacity prism (Prandtl II) | **built** | → [RS2-21](#rs2-21). RS2 SSRM 1.00 vs theory 1.0. |
| 32 | Reinforced embankment, 7 materials (Borges 2002) | **built** | → [RS2-24](#rs2-24). RS2 SSRM 1.24 / 1.21 / 0.98 vs Borges 1.25 / 1.19 / 0.99. |
...
| 39 | Reinforced embankment, geosynthetic (Tandjiria 2002) | **built** | → [RS2-29](#rs2-29). RS2 SSRM 0.97 / 1.42 / 1.22 / 1.39. |
| 40 | Homogeneous, power curve, sensitivity (Perry 1993) | **built** | → [RS2-30](#rs2-30). RS2 SSRM 0.97 vs Perry 0.98. |
| 41 | Homogeneous, power curve, r<sub>u</sub> (Jiang/Baker 2003) | **built** | → [P4-VP41](#p4-vp41) (own SSRM build, 1.647). RS2 SSRM 1.64 vs Bishop 1.66 / Janbu 1.60–1.67. |
...
| 44 | Homogeneous, M-C vs power curve (Baker 2003 ex. 1) | **built** | → [RS2-31](#rs2-31). RS2 SSRM 0.96 / 1.5 / 0.93. |
| 45 | Homogeneous, M-C vs power curve (Baker 2003 ex. 2) | **built** | → [RS2-32](#rs2-32). RS2 SSRM 2.65 / 2.78 / 2.63. |
| 51 | 4 materials, water table, TC, seismic, 12-method (Zhu 2003) | **built** | → [RS2-51](#rs2-51) (LEM, partial). RS2 SSRM 1.22 vs Slide2 Spencer 1.293 / GLE 1.304. |
| 56 | Homogeneous, water table, TC (Pockoski & Duncan slope 2) | **built** | → [RS2-33](#rs2-33). RS2 SSRM 1.26 vs 8-program 1.02–1.32. |
| 57 | Layered, TC (Pockoski & Duncan slope 3) | **built** | → [P4-VP57](#p4-vp57) (own SSRM build, 1.301). RS2 SSRM 1.32 vs 8-program ~1.40. |
...
| 70 | Submerged homogeneous slope (Duncan & Wright Fig 6.27) | **built** | → [P4-VP70](#p4-vp70) (own SSRM build, 1.594). RS2 SSRM 1.58 vs Spencer 1.60, ref 1.60. |
| 71 | Homogeneous, FE seepage (Duncan & Wright Fig 6.37) | **built** | → [RS2-36](#rs2-36). RS2 SSRM 1.11 / 1.12 vs Spencer 1.13 / 1.14. |
| 72 | Embankment dam, 4 materials, FE seepage (D&W Fig 6.39) | **built** | → [RS2-37](#rs2-37). RS2 SSRM 1.00–1.49 vs Spencer 1.16–1.63. |
| 74 | Cohesionless embankment on clay (D&W Fig 7.12) | **built** | → [RS2-38](#rs2-38) (SSRM 1.168). RS2 SSRM 1.17 vs Spencer 1.20. |
| 75 | James Bay dyke, 4 materials (D&W Fig 7.16) | **built** | → [RS2-42](#rs2-42). RS2 SSRM 1.19 vs circ 1.45 / non-circ 1.17. |
| 76 | Homogeneous embankment dam, FE seepage (D&W Fig 7.19) | **built** | → [RS2-40](#rs2-40). RS2 SSRM 0.97 / 0.98 vs ref 1.08–1.19. |
| 78 | Purely cohesive slope, thickness variants (D&W Fig 14.3) | **built** | → [RS2-47](#rs2-47) (all three: 30/46.5/60 ft). SSRM 1.077 / 1.061 / 1.061 vs RS2 SSRM 1.03 / 1.02 / 1.02; D&W 1.12–1.14. |
| 79 | Earth embankment, infinite-slope failure (D&W Fig 14.4) | **built** | → [RS2-41](#rs2-39) (infinite 1.430 / deep 1.419). RS2 SSRM 1.41 / 1.45 vs ref 1.40 / 1.44. |
...
SSRM 1.33, and VP6 (ACADS 2b Talbingo) now carrying a [Part IV SSRM build](#p4-vp6) confined to
...
deterministic core of the [VP36](rocscience.md#vp36) reliability benchmark. The FEM carries the
r<sub>u</sub> option (locked on RS2-14/17b/18b), so the run the row was waiting on is now owed no
longer. As on [RS2-14](#rs2-14), the SSRM factor does not become mesh-independent under
...
| Method | XSLOPE | Published |
|---|---|---|
| SSRM (baseline wall, vp087, T<sub>a</sub> = 10 kN/m) | 0.969 (lock, pre-cap; see below) | L&H 0.99 (FLAC) / 1.00 (Bishop); Slide 1.04 |

**The baseline lock is deliberately held at its pre-cap value and this row is expected to run red.**
The vendor model caps tensile strength on every material, T = 0 on the reinforced granular fill
included; those caps are now carried through transcription and applied. Under them the legacy
non-convergence machinery cannot bracket this wall at all — the trials neither converge nor collapse,
they sit stationary at elastic displacement scale, so bisection has no failure side to work against
and the run aborts. The hybrid failure criterion, which requires displacement evidence before calling
a trial failed, does bracket it, at **0.931**. That number is *not* adopted as the lock: a T = 0 return
map that stalls rather than either standing or failing is an open solver investigation into
zero-tension return mapping, and re-recording against a solver whose behavior in that regime is under
review would bake the symptom into the reference. The lock therefore stays 0.969 and this row carries
the same known-red handling as [RS2-67](#rs2-67) b/e/f until the investigation closes.
...
Capping tensile strength is therefore demonstrably one of the mechanisms separating RS2's
...
sixth of C2's 8.5 % and a tenth of C4's 7.0 % — so the residual divergence stays **recorded, cause not fully
determined**. What has been ruled out is a truncated sweep: the SRF control block of each vendor `.fea`
...
is a vestigial default). No further cause is evidenced in the files, so none is asserted.
...
solves, and behave as one packet. Case 2 (full pool) gives SSRM ≈ 1.60, within the Slide2 LEM
method spread (Janbu 1.55 – Bishop 1.64 – GLE 1.71 – Spencer 1.73) but ~6% below the RS2 SSR
reference (1.70). Case 4 (drawn down to el 7.3) gives SSRM 2.207 downstream and 2.660 upstream,
each again inside the Slide2 method spread (2.09 – 2.35 and 2.52 – 2.79) but −5.7% / −3.6% below
RS2's 2.34 / 2.76. Because each exceeds the corpus vendor-match tolerance, all three own-flow rows carry
**regression locks at XSLOPE's own values** (1.602 / 2.207 / 2.660), with the deltas against the
RS2 SSR column (−5.9% / −5.7% / −3.6%) documented here rather than asserted as vendor matches —
...
Case-3 rows match RS2 to < 1%, and the own transient-flow solver reproduces RS2's 90 h field to
<0.3 m on the upstream face. The residual is **not** the unsaturated curve: a sandbox study
reproduced RS2's actual built-in permeability model (the "Simple" model, "General" soil type —
which Rocscience documents as an approximation for phreatic-surface location) as a tabulated curve
and re-ran the Case-2 chain; the head field is nearly SWCC-invariant (phreatic moves ≤ 0.07 m mean)
and every realistic curve moves FS *down* (1.54–1.60), never toward RS2's 1.70. Case 4 is a
zero-flow equilibrium at the drawn-down pool, so its field is conductivity-independent outright.
...
gets its **own** SSRM run against RS2's published SSRM — the LEM lock alone does not discharge
...
[RS2-29](#rs2-29), whose clay case truly has no crack construct at all, VP2's vendor model
...
B = 0.8, γ = 20 kN/m³), with r<sub>u</sub> = 0.3. Both the non-linear envelope (locked on
RS2-30/31/32/34) and the r<sub>u</sub> option (locked on RS2-14/17b/18b) are already in the SSRM
solver; this is the first corpus problem to run them at once.
...
downstream wedge. Earlier the file built the sand as a stacked profile line, and XSLOPE's
polygon extraction kept only the upstream wedge — dropping the downstream sand (x ≈ 17…225,
