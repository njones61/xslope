...
<0.3 m on the upstream face — so the residual is in the reconstructed steady flow field (RS2's
built-in unsaturated SWCC is not reproducible, and the free surface / seepage face carry the
difference). FS rises monotonically as the dam drains, so the governing minimum across the drawdown
