...
- ✅ **Display dock** — per-plot-type display options live in a context-sensitive "Display" dock under the Inputs tree (not in the run dialogs); its page follows the active result tab and re-renders the cached result live. Panels for every view that has options: Inputs (material table), LEM · Search (highlight critical), LEM · Solution (slice numbers, seep contours), Mesh (nodes / element + node labels), Seep · Data and FEM · Data (BC / nodes / labels), plus the existing Seep · Solution and FEM · Results panels. Each exposes only kwargs the underlying `plot_*` already accepts; Reliability has none, so it shows the placeholder.
