...
### 7.1 Phase 1: DXF Import

DXF is the practical choice for Phase 1:
- Open format, well-documented
- `ezdxf` Python library is mature and well-maintained
- Most commonly exchanged format for cross-section geometry
- AutoCAD, MicroStation, and most CAD software can export DXF

**Key DXF feature**: Every entity in a DXF file belongs to a **layer** (e.g., "CLAY_FILL", "BEDROCK", "SAND_LENS"). In geotechnical practice, CAD drawings typically organize material zones by layer — all polylines representing clay are on the "CLAY" layer, etc. These layer names are valuable for identifying and assigning materials.

**Workflow**: A standalone utility that reads a DXF file, helps the user validate and assign materials, and writes the result to an Excel input template.

**Step 1 — Read DXF**: Extract all closed polylines with their layer names.

```python
def read_dxf_polygons(dxf_path):
    """Read closed polylines from DXF with layer names."""
    import ezdxf
...
**Step 2 — Display for validation**: Plot all polygons with layer names as labels. Print a summary table so the user can verify the import:
...
The plot and table let the user confirm that the correct polylines were extracted, the geometry looks right, and no zones are missing or duplicated.

**Step 3 — Material assignment**: The user maps DXF layers to material IDs. Unique layer names are listed, and the user assigns a material number to each:
...
All polygons on the same layer get the same material ID. This is usually the natural mapping since CAD layers correspond to material zones.

**Step 4 — Write to Excel**: Populate the `polygons` sheet in an existing or new Excel template with Poly ID, Mat ID, and vertex coordinates. Optionally, also populate the `mat` sheet with material names derived from the DXF layer names (e.g., layer "CLAY_FILL" → material name "CLAY_FILL" with default placeholder properties that the user then fills in). This gives the user a head start on the material definitions rather than starting from blank rows.

### 7.2 Future: DWG, DGN

DWG requires Autodesk libraries or the ODA File Converter. DGN is niche. Add based on user demand.
...
8. **CAD import**: DXF import utility
