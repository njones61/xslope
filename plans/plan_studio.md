...
## 14. Importing from other LEM packages — moved

Importing problems from other slope-stability software (GeoStudio SLOPE/W `.gsz`,
Rocscience Slide2, …) onto `slope_data` is now tracked in its own plan:
[`plan_import.md`](plan_import.md) (motivation, target formats, the 2026-06-27
scout findings, and the feasibility-spike next step).
...
- ✅ **Full undo/redo coverage** — snapshot undo/redo is built (deep-copy `slope_data` per edit, wired to the Undo/Redo actions, enable-state refreshed on render). Both the GUI editors (`edit_category` → `begin_edit`/`commit_edit`) **and the AI assistant** (`ai/assistant._run_python` wraps each `run_python` call: snippet raised → `rollback_edit`; inputs changed → `commit_edit`; no-op → `cancel_edit`) land on the same stack, so an assistant that mangles the data is undoable. The hardening below (labeled history, toolbar dropdown, assistant polygon-edit detection, result invalidation, mesh-sharing snapshots, saved-state marker) is complete. *(An editing concern — kept here rather than under DXF.)*
  ...
  **Undo hardening:** make undo legible, complete, and reachable from the assistant path —
  ...
  5. ✅ **Cap stack depth + stop deep-copying the mesh.** `MAX_HISTORY = 100` (oldest dropped, clean-marker shifted with it). `_snapshot()` deep-copies all of `slope_data` **except `mesh`**, whose (potentially large) object is shared by reference — safe because the mesh is only ever replaced wholesale (Build Mesh / clear), never mutated in place — so it still time-travels correctly through undo without the per-snapshot copy cost.
  6. ✅ **Saved-state marker.** A `_clean_index` records the undo position last saved at; `_refresh_dirty()` recomputes dirty as *position ≠ saved* (OR an out-of-band `_style_dirty`, since styles live outside the undo stack), so undo/redo **back to the saved point clears the dirty flag** and the title asterisk. The marker is invalidated when a new edit discards the redo branch holding it, or when it scrolls off the capped stack; New/Open reset it to clean, DXF import to "never saved".
  - Verified headlessly: document-level labeled/multi-step undo/redo, relabel, cap, rollback/cancel, polygon-key detection, the assistant label helper, mesh-shared snapshots (identity + isolation + time-travel), and the saved-state dirty marker (undo/redo-to-clean, branch destruction, style OR), plus a MainWindow smoke test of the toolbar split-buttons + history menus.
...
**Phase 4 — Meshing + Seepage + FEM** ✅ **DONE**
...
- ✅ **Fine-grained FEM progress** — `solve_fem` takes an optional `progress_callback(frac, label)` called (throttled) from inside the viscoplastic loop; each SSRM step (bound checks, bisection halvings, `displacement_increase` sweep + refine) subdivides its slice of the bar (`SUBDIV=100`) and lets that inner fraction fill it, so the bar advances *within* each `solve_fem` trial instead of freezing per bisection step. `frac = (iteration+1)/max_iterations` is a pessimistic denominator that snaps forward on early convergence. Single-trial FEM also drives a determinate bar. Engine-side so notebooks benefit; verified the SSRM FS values are unchanged (all 7 `--fem` regression tests still pass).
...
**Phase 4 — done** (Meshing + Seepage + FEM all wired; display panels for all views; image export incl. DXF; auto legend layout; fine-grained FEM/SSRM progress).
