...
def pick_category(slope_data, x, y, tol, mode=None):
    ...
    # AFTER the material zones and in this order — SSR zones, v21 refine regions, then
    # v27 joint-network regions (PolygonEditor.build merges the four lists the same
    # way) — so the row a click resolves to is offset by the counts of everything
    # ahead of it.
    ...
    for j, zone in enumerate(ssr_zones):
        try:
            ...
            pass
    refine_zones = d.get("refine_zones") or []
    for j, zone in enumerate(refine_zones):
        try:
            ring = LineString(list(zone["polygon"]) + [zone["polygon"][0]])
            ...
            pass
    for j, zone in enumerate(d.get("joint_zones") or []):
        try:
            ring = LineString(list(zone["polygon"]) + [zone["polygon"][0]])
            cands.append((ring.distance(pt), "polygons",
                          len(polygons) + len(ssr_zones) + len(refine_zones) + j))
        except Exception:
            pass
