...
def fig_seep_slope_worked_example():
    """The critical-surface panels of the worked example.

    The model is loaded once. ``load_slope_data`` picks up the mesh and the nodal
    field committed beside it, which is the workflow the page describes, so both
    panels are drawn on the section the reader downloads.

    The **search** and the **critical surface** come from one
    ``circular_search`` — the run the page's tag locks — so the factor of safety
    on the two figures, in the prose and in the tag is one number.
    """
